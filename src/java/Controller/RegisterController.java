...
public class RegisterController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (phoneNumber == null || phoneNumber.trim().isEmpty()) {
            ...
            errors.add("Invalid phone number format. Must be 10 digits and start with 0.");
        } else if (userDAO.isPhoneNumberRegistered(phoneNumber)) {
    ...
    private boolean isValidEmail(String email) {
        String regex = "^[A-Za-z0-9+_.-]+@(gmail\\.com|fpt\\.edu\\.vn)$";
    ...
    private boolean isValidPhoneNumber(String phone) {
        // Kiểm tra định dạng: chỉ chấp nhận số bắt đầu bằng 0, có 10 chữ số
        boolean isValidFormat = phone.matches("^0\\d{9}$");
        // Kiểm tra số điện thoại đã tồn tại trong database hay chưa
        UserDao userDAO = new UserDao(); // ✅ Tạo đối tượng UserDao
        boolean isRegistered = userDAO.isPhoneNumberRegistered(phone); // ✅ Gọi từ UserDao

        return isValidFormat && !isRegistered;
    }
